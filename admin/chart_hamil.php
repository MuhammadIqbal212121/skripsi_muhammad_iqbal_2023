          ...
          <div class="card">
            ...
            <div class="card-body">
              ...
              <div class="card card-success">
                <div class="card-header">
                  <h3 class="card-title"> Chart Ibu Hamil</h3>
...
<?php include '../config/footer.php';
  error_reporting(0);
  $jumlah = array();
  $month = mysqli_query($koneksi, "SELECT DAY(tgl_jadwal) AS tgl, COUNT(id_jadwal_hamil) AS jumlah FROM jadwal_hamil 
WHERE YEAR(tgl_jadwal) = year(curdate()) AND MONTH(tgl_jadwal) = MONTH(curdate())
GROUP BY DAY(tgl_jadwal);");
  if (isset($_POST['filter'])) {
    $tahun = $_POST['tahun'];
    $bulan = $_POST['bulan'];
    $month = mysqli_query($koneksi, "SELECT DAY(tgl_jadwal) AS tgl, COUNT(id_jadwal_hamil) AS jumlah FROM jadwal_hamil 
  WHERE YEAR(tgl_jadwal) = '$tahun' AND MONTH(tgl_jadwal) = '$bulan'
  GROUP BY DAY(tgl_jadwal);");
  }
}
?>
  ...
  var barChartOptions = {
    ...
    scales: {
      ...
      yAxes: [{
        scaleLabel: {
          display: true,
          labelString: 'Jumlah jadwal Perhari',
        },
        ticks: {
          stepSize: 1,
          min: 0,
