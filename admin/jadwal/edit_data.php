<?php
$id = $_GET['id'];
$select = mysqli_query($koneksi, "SELECT * FROM jadwaldokter WHERE id_jadwal = '$id'");
$d = mysqli_fetch_array($select);
if (isset($_POST['save'])) {
  foreach ($_POST as $key => $value) {
    ${$key} = $value;
  }
  $query = mysqli_query($koneksi, "UPDATE jadwaldokter SET id_dokter = '$id_dokter', id_poliklinik = '$id_poliklinik', 
  hari = '$hari', jam_mulai = '$jam_mulai', jam_selesai = '$jam_selesai' WHERE id_jadwal = '$id'");
  if ($query) {
    ?>
    <script>
        ...
        <form method="POST">
            ...
            <div class="form-group">
              ...
              <select name="id_dokter" class="form-control select2" required>
                ...
                while ($data = mysqli_fetch_array($sql)) { ?>
                  <option <?= select($d['id_dokter'], $data[0]) ?> value="<?= $data[0] ?>"><?= $data['nama'] . ' / ' . $data['spesialisasi'] ?></option>
                <?php } ?>
              </select>
            </div>
            <div class="form-group">
