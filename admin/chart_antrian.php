          ...
          <div class="card">
            ...
            <div class="card-body">
              ...
              <div class="card card-success">
                <div class="card-header">
                  <h3 class="card-title"> Chart Antrian</h3>
...
<?php include '../config/footer.php';
  error_reporting(0);
  $jumlah = array();
  $month = mysqli_query($koneksi, "SELECT DAY(tgl_antrian) AS tgl, COUNT(id_antrian) AS jumlah FROM antrian 
WHERE YEAR(tgl_antrian) = year(curdate()) AND MONTH(tgl_antrian) = MONTH(curdate())
GROUP BY DAY(tgl_antrian);");
  if (isset($_POST['filter'])) {
    $tahun = $_POST['tahun'];
    $bulan = $_POST['bulan'];
    $month = mysqli_query($koneksi, "SELECT DAY(tgl_antrian) AS tgl, COUNT(id_antrian) AS jumlah FROM antrian 
  WHERE YEAR(tgl_antrian) = '$tahun' AND MONTH(tgl_antrian) = '$bulan'
  GROUP BY DAY(tgl_antrian);");
  }
}
?>
  ...
  var barChartOptions = {
    ...
    scales: {
      ...
      yAxes: [{
        scaleLabel: {
          display: true,
          labelString: 'Jumlah Antrian Perhari',
        },
        ticks: {
          stepSize: 1,
          min: 0,
